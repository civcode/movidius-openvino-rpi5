...
correctness. **No code changes made in this pass** — findings only, as a
todo list for a follow-up fix pass.
...
- [ ] **BUG (hardening)** `MyriadClient.infer()` raises a bare
...
- [ ] **BUG (hardening)** `preprocess()` uses `cv2` without a `cv2 is None`
...
- [ ] **NIT** `close()` closes `proc.stdin`/`proc.stdout` even when the
...
- [ ] **BUG (empirically confirmed, common path)** `--file *.ppm` mode
...
- [ ] **BUG (crash on 14 COCO classes)** `detect()` splits `DET` lines on
...
- [ ] **NIT** `--file foo.jpg` with `--headless` and OpenCV missing passes
...
- [ ] **DOC (header comment, wrong)** header says the client feeds "raw
...
- [ ] **DOC (`--help`/usage incomplete)** `usage()` shows only the
...
- [ ] **PERF/CLARITY (single-image mode)** the resize is executed twice:
...
- [ ] **NIT** `outElemBytes`/`outElems` are read from the blob *before*
...
- [ ] **BUG (self-containedness)** `roundToI()` uses `std::floor` but the
...
- [ ] **INCONSISTENCY** this launcher's auto has the docker-CLI check and
...
- [ ] **DOC (output layer name)** input/output table names the output
      `detection_boxes/sink_port_0`; the app prints `DetectionOutput
      [1,1,100,7]`. Use the actual blob name.
- [ ] **DOC (missing cgroup rule)** the webcam-stream section says 'run
...
- [ ] **NIT** "Options mirror the single-image mode (`--min-conf`,
...
- [ ] **BUG (latent UB)** the output conversion loop writes `n` elements
...
- [ ] **NIT** `outElems = getDims().back()` assumes the class count is the
...
- [ ] **INCONSISTENCY (cross-app)** `mobilenet_server` always reads stdin
...
- [ ] **DOC GAP (in-container camera runs)** verified empirically: with
...
- [ ] **BUG/DOC conflict: in-image fallback unreachable via `auto`** the
...
- [ ] **NIT** `-h|--help` is only recognized as `$1` (any later arg
...
- [ ] **BUG (robustness)** a single failed `cap.read()` calls `die()` —
      real webcams occasionally drop a frame (`ok=False`); tolerate N
      consecutive failures (e.g. 5) before aborting.
- [ ] **NIT (status line conventions)** verified: `webcam_mobilenet.py`
...
- [ ] **BUG (`--mask-out` writes an invalid PPM)** the client writes the
...
- [ ] **BUG/UX (`--file` silently resizes to 640x480)** `--file img.ppm`
...
- [ ] **INCONSISTENCY (no `--backend`/`--device`/`--request-timeout`)**
...
- [ ] **NIT** the client handles an `ERROR <msg>` protocol line the server
...
- [ ] **NIT** `CLASS <id> <name> <pixels>` parsing assumes single-word
...
- [ ] **NIT** headless fps here is *instantaneous* (`1/dt` per frame)
...
- [ ] **BUG (bilinear upscale overshoot → uint8 wrap)** `bilinearResize`
      ...
      free).
- [ ] **DOC (terminology)** the header says 'this client feeds raw 0..255
...
- [ ] **DOC (`--help` is terse)** the one-line usage prints both modes
...
- [ ] **NIT** `FRAME <total_ms>` excludes the nearest-resize, histogram
      ...
      field.
- [ ] **NIT** logits branch is unreachable here: `seg_detect` gates
...
- [x] Self-contained includes (has `<cmath>`, unlike
      ...
      histogram sort.
- [ ] **INCONSISTENCY** `seg::loadLabels` silently accepts an empty or
...
- [ ] **DOC GAP (minor)** usage repeats the protocol block but, like the
...
- [ ] **DOC (wrong sentence about who resizes)** "The client therefore
...
- [ ] **DOC (stats channel)** "class statistics go to stderr/overlay
      text" — the headless client prints them to **stdout**; only server
      diagnostics are stderr.
- [ ] **DOC (overstated feature)** 'Output is interpreted from its shape:
...
- [ ] **DOC GAP** the streaming section shows the plain
...
- [ ] **BUG (`--show-errors` lists top-5 misses, not top-1 misses)** the
...
- [ ] **DOC (docstring expectation contradicts verified numbers)** the
...
- [ ] **NIT** file discovery globs `*.JPEG`, `*.jpg`, `*.png` but not
...
- [ ] **BUG/UX (destructive re-run)** a second run when `images/` exists
...
- [ ] **DOC (stale path)** 'The conversion now lives in
...
- [ ] **DOC (`--show-errors` wording)** the bullet 'a list of up to
...
- [ ] **Launcher trio alignment**: `infer-ssd-server.sh` and
      ...
      extracted for C++.
- [ ] **Client CLI conventions**: webcam client = positional script +
...
- [ ] **Protocol error signalling**: only `seg_stream.py` handles an
...
- [ ] **`half.hpp`/`device_probe.hpp` include style is Docker-only**:
...
- [ ] **Duplication candidates for the next refactor round**
